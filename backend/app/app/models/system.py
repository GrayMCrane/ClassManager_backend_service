...
class Region(Base):
    ...
    """
    id = Column(Integer, primary_key=True, comment='地区id，主键')
    name = Column(String(20), nullable=False, comment='地区名称')
    code = Column(String(10), nullable=False, comment='行政区编号')
    abbr = Column(String, comment='省级行政区名称缩写')
    parent_id = Column(Integer, comment='父级地区id')
    level = Column(
        Integer,
        nullable=False,
        comment='地区级别: '
...
class SysConfig(Base):
    ...
    _type = Column('type', Integer, nullable=False, comment='配置类型')
